...
public class InvenDAO {
	...
	// 책 구매 - 도서관
	public int insertdBook(InvenDTO dto) {
		int result = -1;
		...
		try {
			...
			// SQL 준비
			String query = " insert into li_book (book_code, li_book_info, book_isbn, book_loan, book_res, book_loan_seq) ";
			query += " values(seq_li_book.nextval, ?, ?, 'N', 'N', 0) ";
			PreparedStatement ps = new LoggableStatement(con, query);
			
			ps.setString(1, dto.getLi_book_info());
			ps.setLong(2, dto.getBook_ISBN());
	...
	public int insertISBN(InvenDTO dto) {
		...
		try {
			...
			// SQL 준비
			String query = " INSERT INTO book (book_isbn, book_name, book_author, book_pub, book_img) ";
			query += "VALUES (?,?, ?, ?, ?) ";
					
			PreparedStatement ps = new LoggableStatement(con, query);
			ps.setLong(1, dto.getBook_ISBN());
			ps.setString(2, dto.getBook_name());
			ps.setString(3, dto.getBook_author());
			ps.setString(4, dto.getBook_pub());
			ps.setString(5, dto.getBook_img());
